...
export default function SearchBar() {
    ...
    try {
      ...
      const resultsWithContent = await Promise.all(
        searchResults.map(async (result) => {
          ...
          // Find the chunk
          const chunk = document.content.chunks.find(c => c.id === result.chunkId);
          ...
          const contextText = extractContextAroundMatch(chunk.text, searchQuery);
  ...
  const handleLanguageToggle = (lang: 'english' | 'hindi' | 'hinglish' | 'all') => {
    setLanguage(lang);
  };
